...
44. The dashboard promotes the binary observing-safety sensor to a plain-language hero state, keeps compact controls at least 40px square, hides decorative sparklines from assistive technology, and serves precompiled Tailwind utilities from `tailwind.generated.css`.
45. The visual system uses `celestial-map.svg` as subtle, non-interactive astronomical navigation linework across page backgrounds and selected surfaces; it should remain thin, low-contrast, and secondary to telemetry.
46. This supersedes the strict single-viewport height target in decision 43: the desktop telemetry and media region is 35% taller to reduce visual compression, so shorter laptop viewports may scroll slightly.
47. Sensor-card sparklines use native, non-interactive SVG paths with screen-reader trend summaries; Highcharts remains limited to analytical and historical charts that require axes, zooming, ranges, or richer interaction.
48. Live sensor readings become stale 150 seconds after their last MQTT message, camera frames become stale after five minutes, and stale or disconnected safety telemetry must display an unknown observing state rather than the last safe/unsafe value.
